...
const MyApplications = () => {
  ...
  const filteredApplications = userApplications.filter((app) => {
    ...
    var companyMatch;
    if (app.jobId) {
      titleMatch = app.jobId.title
        .toLowerCase()
        .includes(searchTerm.toLowerCase());
      companyMatch = app.jobId.company
        .toLowerCase()
        .includes(searchTerm.toLowerCase());
    } else {
      titleMatch = "Job has been deleted".includes(searchTerm.toLowerCase());
      companyMatch = "Job has been deleted"
        .toLowerCase()
        .includes(searchTerm.toLowerCase());
    }
              ...
              <div>
                <h3
                  className={`text-xl font-semibold ${
                    app.jobId ? "text-indigo-700" : "text-red-700"
                  }`}
                >
                  {app.jobId ? app.jobId.title : "Job has been deleted"}
                </h3>
                <p className="text-gray-600">
                  {app.jobId ? app.jobId.company : "Job has been deleted"}
                </p>
                ...
                <button
                  ...
                  className={`text-sm flex items-center gap-1 hover:underline ${
                    app.jobId
                      ...
                      : "text-red-700 line-through hover:opacity-0 hover:cursor-not-allowed"
                  }`}
                  disabled={app.jobId ? false : true}
                >
